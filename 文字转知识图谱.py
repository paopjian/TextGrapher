...
class CrimeMining:
    ...
    def extract_triples(self, words, postags, ner):
    ...
    def main(self, content):
        ...
        for sent in sents:
            ...
            words_list += [[i[0], i[1]] for i in zip(words2, postags2)]
            subsents_seg.append(words2)
            ner = self.collect_ners(words2, postags2)
            if ner:
                triple = self.extract_triples(words2, postags2, ner)
                if triple:
                    triples += triple
                    ners += ner
                    ner_sents.append([words2, postags2])
        # 获取文章关键词, 并图谱组织, 这个可以做
        keywords = self.parser.keyword.extract_tags2(words=words_list, topK=10,
                                                     allowPOS=('n', 'nr', 'ns', 'nz', 'PER', 'nt', 'ORG', 'LOC', 'vn'))
        for keyword in keywords[0]:
            if keywords[1][keyword] > 1:
                name = keyword
                cate = '关键词'
                events.append([name, cate])
        # 对三元组进行event构建，这个可以做
        for t in triples:
            if (t[0] in keywords[0] or t[1] in keywords[0]) and len(t[0]) > 1 and len(t[1]) > 1:
                events.append([t[0], t[1]])
        # 获取文章词频信息话，并图谱组织，这个可以做
        for word, num in keywords[1].items():
            if num > 1:
                name = word
                cate = '高频词'
                events.append([name, cate])
        # 　获取全文命名实体，这个可以做
        ner_dict2 = {i[0]: i[1] for i in Counter(ners).most_common()}
        for ner in ner_dict2:
            name = ner.split('/')[0]
            cate = self.ner_dict[ner.split('/')[1]]
            events.append([name, cate])
        # 获取全文命名实体共现信息,构建事件共现网络
        co_dict = self.collect_coexist(ner_sents, list(ner_dict2.keys()))
        co_events = [[i.split('@')[0].split('/')[0], i.split('@')[1].split('/')[0]] for i in co_dict]
        events += co_events
        # 将关键词与实体进行关系抽取
        events_entity_keyword = self.rel_entity_keyword(ners, keywords[0], subsents_seg)
        events += events_entity_keyword
        # 对事件网络进行图谱化展示
        self.graph_shower.create_page(events, 'graph_show.html')
        print(events)
